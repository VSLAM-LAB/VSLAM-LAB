...
`<source>` maps to one of these download labels: `website` · `hugging-face` · `google-drive` · `local` · `api` · `other` (anything else — ask the user) — canonical definitions in `Datasets/extra-files/closed_lists.md#download-type-download`. Step 1's "From `<source>`" resolution rule (item 2 below) classifies `<source>` against these for real — not restated here.
...
@../../../Datasets/extra-files/closed_lists.md
...
- `cam_models`, `raw_formats`, `modes` — as YAML lists, e.g. `['pinhole']`, `['ros1', 'zip']`, `['mono', 'stereo']`. The template above lists the closed-list values; the full mapping for all three is canonical in `closed_lists.md` (already included in step 1) — gotchas for `cam_models` are in step 4's template's `create_calibration_yaml` WARNINGS.
- The download field for the step-1 `download` pattern — the template above lists the exact field name/shape; the fetch mechanism is canonical in `closed_lists.md` (already included in step 1).
...
| e. `create_calibration_yaml` | The written `cam_model` must equal step 1's resolved value, as one indivisible choice (never two separately-returned values that can diverge); cast parsed numerics to `float(...)` before writing; if `create_rgb_folder` resizes (`self.target_resolution` set), rescale `focal_length`/`principal_point` to match via `utilities.scale_intrinsics` — don't write them straight from the raw calibration source's native resolution ([#99](https://github.com/VSLAM-LAB/VSLAM-LAB/issues/99)) | `dataset_youtube.py`, `dataset_kitti.py`, `dataset_hilti2026.py` |
