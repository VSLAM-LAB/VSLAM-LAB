...
| `README.md` | Step 7 | Tools-table row hand-added directly; Datasets-table row instead regenerated by running `Datasets/extra-files/generate_readme_datasets_table.py` — nothing else in the file changes either way |
...
| `cam_models` | closed list — `Datasets/extra-files/closed_lists.md#camera-models-cam_models` |
| `raw_formats` | closed list — `Datasets/extra-files/closed_lists.md#raw-format-raw_formats` |
| `modes` | closed list — `Datasets/extra-files/closed_lists.md#modes-modes` |
...
| `calibration_type` | closed list — `Datasets/extra-files/closed_lists.md#calibration-type-calibration_type` (`global` or `per-sequence`) |
| `download` | closed list — `Datasets/extra-files/closed_lists.md#download-type-download` (one of `website`/`hugging-face`/`google-drive`/`local`/`api`) |
| `download_issues` | closed list — `Datasets/extra-files/closed_lists.md#download-issues-download_issues` (one of `complete_dataset`/`api_token`/`huggingface_token`/`license_required`, or blank) |

**`modes` applies dataset-wide, not per-sequence** — `DatasetVSLAMLAB.check_sequence_integrity()` requires *every* sequence to satisfy *every* listed mode (e.g. an `rgb_1` folder for every sequence if `stereo`/`stereo-vi` is listed), regardless of whether that sequence's own source data supports it. If sequences have genuinely different capabilities, split into separate `dataset_name`s by capability instead of forcing the richest `modes` list on all of them — repeat steps 3–9 per split. Model: `dataset_rover.py`'s `rover-t265`/`rover-d435i`/`rover-picam` (by sensor type); `dataset_pamir.py`/`dataset_pamir_rig.py`'s `pamir`/`pamir-rig` (2024 rig dive vs. 2025 single-camera dives) — the split there was only caught in step 8, after a stereo sequence had already broken a mono-only dataset's `check_sequence_availability`. Catch it here instead.
...
Using `Datasets/extra-files/dataset_table.md`, find datasets sharing at least one label with the step-1 table — every field except `dataset_name`/`sequence_names`. Five of them (`cam_models`, `raw_formats`, `modes`, `download`, `download_issues`) are columns in the table, so scan it directly. The other three (`resize`, `groundtruth_available`, `calibration_type`) aren't table columns — check those only among datasets already surfaced by the five table-based labels, by reading that candidate's `.yaml`/`.py` directly: `resize` ↔ its `.yaml` sets `target_resolution`; `groundtruth_available` ↔ its `create_groundtruth_csv` does real parsing rather than writing a header-only file; `calibration_type` ↔ its `create_calibration_yaml` uses a global constant vs. parses a per-sequence file (`Datasets/extra-files/closed_lists.md#calibration-type-calibration_type`). Show the user a comparison table of just the overlaps:

| Dataset | Shared Camera Model | Shared Raw Format | Shared Modes | Shared Resize | Shared Groundtruth Available | Shared Calibration Type | Shared Download | Shared Download Issues |
|---|---|---|---|---|---|---|---|---|
| kitti | pinhole | zip | mono | | | per-sequence | website | complete_dataset |
| ariel | | ros1 | mono-vi stereo-vi | | | | hugging-face | |
...
| Raw Format | `create_rgb_folder` (turning the raw shape into `rgb_0`/`rgb_1`/`depth_0`) |
| Modes | `create_rgb_folder`/`create_rgb_csv`/`create_groundtruth_csv` + folder layout |
| Resize | `create_rgb_folder`'s `self.target_resolution` branch (`compute_scaled_size` + LANCZOS resize path) |
| Groundtruth Available | `create_groundtruth_csv` (real parsing vs. header-only write) |
| Calibration Type | `create_calibration_yaml` (global constant vs. per-sequence file parsing) |
...
**`remove_unused_files` — retention tiers.** `BENCHMARK_RETENTION` (`path_constants.py`, default `Retention.STANDARD`) closed list — the three tiers' exact meanings and the `if BENCHMARK_RETENTION != Retention.FULL` / `== Retention.MINIMAL` code shape are canonical in `Datasets/extra-files/closed_lists.md#benchmark-retention-benchmark_retention`. Every implementation gotcha (unlink-path mismatches, `shutil.rmtree` vs. `unlink()`, symlinked raw folders, and the four shared-archive scopes — whole-dataset-only, scene/group-scoped, dataset-wide indefinitely reused, exact-file share) is canonical in `dataset_template.py`'s `remove_unused_files` comment (included above). Not restated here.
...
### Step 7 — Add the dataset to README.md

**Pick the table first.** `README.md` has two tables for this mechanism, updated two different ways:
- **Datasets** — a fixed, published benchmark with its own sequences (groundtruth typically shipped or derivable). This is the generated `<table>...</table>` HTML block — regenerated from every `dataset_*.yaml`'s `about:` block, never hand-edited.
- **Tools** — a data-capture app/format whose sequences the user brings themselves (e.g. `strayscanner`; no fixed published sequence set). This is a plain markdown pipe table, hand-edited.
...
**If Datasets table:** confirm step 3's YAML has `about.features` populated — closed list, canonical in `Datasets/extra-files/closed_lists.md#dataset-features-aboutfeatures`. Ask the user if unclear, don't guess. Then run:
```
python3 Datasets/extra-files/generate_readme_datasets_table.py
```
This rewrites the entire `<table>` block in place from `about.features`/`about.summary`/`about.homepage`/`modes`/`cam_models` across all datasets — it is not additive by hand; don't insert a row yourself.

**If Tools table:** append one row as the last real entry — immediately above that table's commented-out placeholder rows for not-yet-implemented entries:
```
...
- `<feature emoji(s)>` — closed list, same as the Datasets table (`Datasets/extra-files/closed_lists.md#dataset-features-aboutfeatures`). Ask the user if unclear, don't guess.
