...
| vrs            | shells out to a separate pixi environment (`pixi run -e <env> python3 <worker_script>`) since the parsing library (projectaria_tools) isn't installable in the main vslamlab env — no shared utility for this yet (unlike ros1/ros2's run_rosbag_frame_extraction), each dataset hand-rolls its own worker script; see [#120](https://github.com/VSLAM-LAB/VSLAM-LAB/issues/120) | dataset_aria_digital_twin.py                               |
...
| cdn_links_file       | requires a per-user links/credentials file (not a token, not a plain URL) issued after accepting a license, that the user must obtain and place at a specific local path themselves — checked with e.g. `if self.cdn_urls_file.exists(): return []`. Distinct from `api_token`/`huggingface_token`: nothing gets pasted into the dataset's own `.yaml`, a whole separate file is placed in the benchmark dir instead. `user action required` (yellow) | `website`, `target_file`     | dataset_aria_digital_twin.py                                                                |
...
| MINIMAL  | delete everything STANDARD does, plus the original source downloads too — only the standardized layout remains: smallest footprint, but not reprocessable without a fresh download                | dataset_eth.py (downloaded `{sequence}_{mode}.zip`), `HFColmapDatasetMixin` in dataset_soneva.py (`rgb_0_raw/`, `rgb_1_raw/`) |
